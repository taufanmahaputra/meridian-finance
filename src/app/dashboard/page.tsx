...
export default function DashboardPage() {
  ...
  const kpis = [
    { icon: <span>💰</span>, iconBg: 'bg-indigo-50', label: t('dashboard.kpi.netSavings'), value: fmt(m.savings, currency), spark: sparkWindow.map((x) => x.savings), ...getTrendData(m.savings, p?.savings ?? null) },
    { icon: <span>📊</span>, iconBg: 'bg-emerald-50', label: t('dashboard.kpi.savingsRate'), value: fmtPct(m.savingsRate), spark: sparkWindow.map((x) => x.savingsRate), ...getTrendData(m.savingsRate, p?.savingsRate ?? null) },
    { icon: <span>💳</span>, iconBg: 'bg-red-50', label: t('dashboard.kpi.totalExpenses'), value: fmt(m.expenses, currency), spark: sparkWindow.map((x) => x.expenses), ...getTrendData(m.expenses, p?.expenses ?? null, true) },
    { icon: <span>📈</span>, iconBg: 'bg-amber-50', label: t('dashboard.kpi.budgetUtil'), value: fmtPct(m.budgetUtil), spark: sparkWindow.map((x) => x.budgetUtil), ...getTrendData(m.budgetUtil, p?.budgetUtil ?? null, true) },
  ];
    ...
    <>
      ...
      <div className="p-4 sm:p-7 max-w-[1440px]">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          {kpis.map((k) => (
            <KpiCard key={k.label} icon={k.icon} iconBg={k.iconBg} label={k.label} value={k.value} trendText={k.text} trendClassName={k.className} sparkline={k.spark} sparklineGood={k.className.includes('emerald')} />
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[2fr_1fr] gap-4 mb-6">
          <Card>
            ...
            <CardBody><ExpenseTrendChart months={months} monthlyBudget={monthlyBudget} currency={currency} /></CardBody>
          </Card>
          <Card>
        ...
        <Card className="mb-6">
          <CardHeader>{t('dashboard.topCategories')}</CardHeader>
          <CardBody><TopCategoriesBar cats={m.cats} catColors={catColors} catBudgets={catBudgets} currency={currency} /></CardBody>
        </Card>
